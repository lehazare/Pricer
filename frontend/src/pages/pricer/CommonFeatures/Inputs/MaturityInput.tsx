...
export default function MaturityInput({ col, rowIndex, rows, setRows }: Props) {
  ...
  const fetchStrikeDate = async (maturity: string) => {
    try {
      ...
      console.error("Erreur lors de la récupération de la strikeDate :", error);
    }
  };

  useEffect(() => {
    if (col.defaultValue && !row.strikeDate) {
      fetchStrikeDate(col.defaultValue);
    }
  }, [col.defaultValue]);
